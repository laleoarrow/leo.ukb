...
role_table <- data.frame(
  变量 = c("outcome", "outcome_censor", "age", "sex", "ecog_group"),
  类型 = c(
    "二分类事件指示变量",
    "以年为单位的随访时间",
    "连续 exposure",
...
在这个示例里，Model A 提示较差的 performance status 与更高的 incident hazard
相关：
...
3. 如果点估计本身就接近 1，那么无论 P 值如何，主关联证据都不会很强
...
> 与 `ECOG0` 相比，较差的 performance status 与更高的 incident hazard 相关，
